...
const InventoryContent = () => {
  ...
  const handlePrintPriceList = () => {
    ...
    doc.setFontSize(16);
    doc.text("Inventory Price List", 14, 14);
    doc.setFontSize(10);
  ...
  const handleThermalPrintPriceList = () => {
    ...
    if (!printWindow) {
      showSnackbar("Allow popups to print thermal list", "error");
      return;
    }
    ...
    const html = `
      <html>
        <head>
          <title>Inventory Thermal Price List</title>
          <style>
            body {
        ...
        <body>
          <p class="title">Inventory Price List</p>
          <p class="meta">${new Date().toLocaleString("en-IN")}</p>
          <table>
            <thead>
              <tr>
                <th>Item Name</th>
            ...
            <tbody>
              ${rows || `<tr><td colspan="2">No items found</td></tr>`}
            </tbody>
          </table>
          <p class="footer">End of list</p>
        </body>
      </html>
    `;
